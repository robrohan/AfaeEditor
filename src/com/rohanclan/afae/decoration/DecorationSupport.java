 ...
 * @since 2.1
 */
public class DecorationSupport extends SourceViewerDecorationSupport implements IPreferenceConstants {
	/**
	...
	// private AfaeCursorLinePainter fCursorLinePainter;
	 ...
	 */
	public DecorationSupport(ISourceViewer sourceViewer,
	...
	public void install(IPreferenceStore store) {
		...
		if (fPreferenceStore != null) {
			try {
				...
				// this can cause and NPE if the editor is closed while the
				// prefernce pane is open and a theme is changed
			}
	...
	private void updateTextDecorations() {
		...
		if (areWhitespaceCharactersShown())
	...
	protected void handlePreferenceStoreChanged(PropertyChangeEvent event) {
		...
		if (fMatchingCharacterPainterEnableKey != null
				&& fMatchingCharacterPainterEnableKey.equals(p)
				&& fCharacterPairMatcher != null) {
			if (areMatchingCharactersShown())
				showMatchingCharacters();
			else
				hideMatchingCharacters();
		...
		if (fMatchingCharacterPainterColorKey != null
				&& fMatchingCharacterPainterColorKey.equals(p)) {
		...
		if (P_SHOW_WHITESPACE != null && P_SHOW_WHITESPACE.equals(p)) {
			if (areWhitespaceCharactersShown())
				showWhitespaceCharacters();
			else
				hideWhitespaceCharacters();
		...
		if (fCursorLinePainterEnableKey != null
				&& fCursorLinePainterEnableKey.equals(p)) {
			if (isCursorLineShown())
				showCursorLine();
			else
				hideCursorLine();
		...
		if (fCursorLinePainterColorKey != null
				&& fCursorLinePainterColorKey.equals(p)) {
		...
		if (fMarginPainterEnableKey != null
				&& fMarginPainterEnableKey.equals(p)) {
			if (isMarginShown())
				showMargin();
			else
				hideMargin();
		...
		if (fMarginPainterColorKey != null && fMarginPainterColorKey.equals(p)) {
			if (fMarginPainter != null) {
				fMarginPainter
						.setMarginRulerColor(getColor(fMarginPainterColorKey));
				fMarginPainter.paint(IPainter.CONFIGURATION);
			}
			return;
		}

		if (fMarginPainterColumnKey != null
				&& fMarginPainterColumnKey.equals(p)) {
			if (fMarginPainter != null && fPreferenceStore != null) {
				fMarginPainter.setMarginRulerColumn(fPreferenceStore
						.getInt(fMarginPainterColumnKey));
				fMarginPainter.paint(IPainter.CONFIGURATION);
			}
			return;
		}
		...
		if (info != null) {
			...
			if (info.getColorPreferenceKey().equals(p)) {
				...
				if (fAnnotationPainter != null) {
					fAnnotationPainter.setAnnotationTypeColor(info
			...
			if (info.getTextPreferenceKey().equals(p)
				...
				Object style = getAnnotationDecorationType(info
						.getAnnotationType());
	...
	private Color getColor(String key) {
		if (fPreferenceStore != null) {
			// if there is no key we got here in some odd way log it
			// and return black so we dont blow up
			if (key != null) {
				...
				try {
					...
					AfaePlugin.logError("I was asked for unknown key: " + key,
							e, DecorationSupport.class);
					// e.printStackTrace();
				}
			} else {
				System.err.println("I was asked for a null key");
		...
		if (fMatchingCharacterPainter == null) {
			if (fSourceViewer instanceof ITextViewerExtension2) {
				fMatchingCharacterPainter = new MatchingCharacterPainter(
						fSourceViewer, fCharacterPairMatcher);
				fMatchingCharacterPainter
						.setColor(getColor(fMatchingCharacterPainterColorKey));
	...
	private void hideWhitespaceCharacters() {
		if (fWhitespacePainter != null) {
			if (fSourceViewer instanceof ITextViewerExtension2) {
				ITextViewerExtension2 extension = (ITextViewerExtension2) fSourceViewer;
	...
	private boolean areWhitespaceCharactersShown() {
		if (fPreferenceStore != null && P_SHOW_WHITESPACE != null)
			return fPreferenceStore.getBoolean(P_SHOW_WHITESPACE);
		return false;
	}
	...
	private boolean areMatchingCharactersShown() {
		if (fPreferenceStore != null
				&& fMatchingCharacterPainterEnableKey != null)
			return fPreferenceStore
					.getBoolean(fMatchingCharacterPainterEnableKey);
		...
		try {
			if (fCursorLinePainter == null) {
				if (fSourceViewer instanceof ITextViewerExtension2) {
					...
					// fCursorLinePainter = new
					// AfaeCursorLinePainter(fSourceViewer);
					fCursorLinePainter
			...
			AfaePlugin.logError("showCursorLine", npe, DecorationSupport.class);
			// npe.printStackTrace();
		}
	}
			...
			if (fSourceViewer instanceof ITextViewerExtension2) {
				if (fFontPropertyChangeListener != null) {
					if (JFaceResources.getFontRegistry() != null) {
						...
						ITextViewerExtension2 extension = (ITextViewerExtension2) fSourceViewer;
						if (extension != null && fMarginPainter != null) {
	 ...
	 *            the annotation type
	 * @param highlighting
	 *            <code>true</code> if highlighting <code>false</code> if
	 *            painting squiggles
	 * @param updatePainter
	...
	private void hideAnnotations(Object annotationType, boolean highlighting,
		...
		if (fAnnotationPainter != null) {
			try {
				if (highlighting)
					fAnnotationPainter
							.removeHighlightAnnotationType(annotationType);
				else
					fAnnotationPainter.removeAnnotationType(annotationType);
			} catch (NullPointerException e) {
				AfaePlugin.logError("hideAnnotations ", e,
						DecorationSupport.class);
			}
		...
		if (fPreferenceStore != null) {
			...
			if (info != null)
				return info.getHighlightPreferenceKey() != null
						&& fPreferenceStore.getBoolean(info
	...
	private boolean isAnnotationOverviewShown(Object annotationType) {
		if (fPreferenceStore != null && fOverviewRuler != null) {
			AnnotationPreference info = (AnnotationPreference) fAnnotationTypeKeyMap
					.get(annotationType);
			if (info != null)
				return fPreferenceStore.getBoolean(info
